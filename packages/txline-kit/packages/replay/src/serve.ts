...
import { corpusDirFor, readFrames, type CorpusFrame } from "./corpus.js";
...
interface Playback {
  /** Virtual timeline position (ms, corpus timestamps). */
  virtualAt(): number;
  seek(toTs: number): void;
  setSpeed(s: number): void;
  pause(): void;
  resume(): void;
  status(): { position: number; speed: number; paused: boolean; start: number; end: number };
}

function createPlayback(startTs: number, endTs: number, speed: number): Playback {
...
export function startReplayServer(opts: ServeOptions): http.Server {
  ...
  function sseHandler(frames: CorpusFrame[], res: http.ServerResponse, fixtureFilter?: number) {
    res.writeHead(200, {
    ...
    if (cursor === -1) cursor = filtered.length;
    let index = 0;
  ...
  const server = http.createServer((req, res) => {
    ...
    if (url.pathname === "/api/scores/stream") return sseHandler(scores, res, fixtureFilter);
    if (url.pathname === "/api/odds/stream") return sseHandler(odds, res, fixtureFilter);

    const scoresSnapshot = url.pathname.match(/^\/api\/scores\/snapshot\/(\d+)$/);
    if (scoresSnapshot) {
      const vnow = playback.virtualAt();
      const past = scores.filter((f) => f.ts <= vnow);
      if (past.length === 0) return send(404, { error: "no updates yet at current replay position" });
    ...
    if (oddsSnapshot) {
      ...
      const latest = new Map<string, CorpusFrame>();
      for (const f of odds) {
        if (f.ts > vnow) break;
        const d = f.data as Record<string, unknown>;
    ...
    if (url.pathname === "/control" && req.method === "POST") {
      ...
      req.on("end", () => {
        try {
          const { action, value } = JSON.parse(body || "{}") as { action: string; value?: number };
          if (action === "pause") playback.pause();
          else if (action === "resume") playback.resume();
          else if (action === "seek" && typeof value === "number") playback.seek(value);
          else if (action === "speed" && typeof value === "number") playback.setSpeed(value);
    ...
    if (url.pathname === "/control") return send(200, playback.status());
    if (url.pathname === "/healthz") return send(200, { ok: true, fixtureId: opts.fixtureId });
