...
export class AuthManager {
    ...
    async renewJwt() {
        ...
        this.renewing = (async () => {
            try {
                ...
                if (!res.ok) {
                    throw new TxlineAuthError(`Guest session failed: HTTP ${res.status} from ${this.opts.network.authUrl}`);
                }
    ...
    async ensureActivated() {
        ...
        if (!wallet) {
            throw new TxlineAuthError("No API token available and no wallet provided. Pass `wallet` (a funded " +
    ...
    async subscribeOnChain(wallet) {
        ...
        if (!accountInfo) {
            const createTx = new web3.Transaction().add(spl.createAssociatedTokenAccountInstruction(wallet.publicKey, userTokenAccountAddress, wallet.publicKey, tokenMint, spl.TOKEN_2022_PROGRAM_ID, spl.ASSOCIATED_TOKEN_PROGRAM_ID));
            await web3.sendAndConfirmTransaction(connection, createTx, [wallet], {
                commitment: "confirmed",
            });
        }
        const [pricingMatrixPda] = web3.PublicKey.findProgramAddressSync([Buffer.from("pricing_matrix")], program.programId);
        const [tokenTreasuryPda] = web3.PublicKey.findProgramAddressSync([Buffer.from("token_treasury_v2")], program.programId);
        const tokenTreasuryVault = spl.getAssociatedTokenAddressSync(tokenMint, tokenTreasuryPda, true, spl.TOKEN_2022_PROGRAM_ID);
        ...
        await connection.confirmTransaction({ signature: txSig, blockhash: latest.blockhash, lastValidBlockHeight: latest.lastValidBlockHeight }, "confirmed");
        return txSig;
    }
    /** POST /api/token/activate with detached signature over "txSig:leagues:jwt". */
    async activate(txSig, wallet) {
        ...
        if (!token)
            throw new TxlineAuthError("Activation returned an empty token");
        this.apiToken = token;
    }
    load() {
        try {
            return JSON.parse(fs.readFileSync(this.credsFile, "utf8"));
        }
        catch {
            return null;
        }
