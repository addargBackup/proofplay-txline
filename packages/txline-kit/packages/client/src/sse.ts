...
export async function* sseStream<T = unknown>(opts: SseStreamOptions): AsyncGenerator<SseMessage<T>> {
  ...
  try {
    ...
    const reader = currentReader as ReadableStreamDefaultReader<Uint8Array> | null;
    await reader?.cancel().catch(() => {});
  }

  async function* run(): AsyncGenerator<SseMessage<T>> {
    for (;;) {
